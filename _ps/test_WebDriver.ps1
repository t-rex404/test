# WebDriverクラステストファイル
# WebDriverクラスの各メソッドの機能をテストするためのスクリプト
...
<body>
    <h1 id="main-title">WebDriver Test Page</h1>
    <div class="test-class">This is a test div</div>
...
# テスト用HTMLファイルを保存
$testHtmlPath = Join-Path $PSScriptRoot "test_page.html"
$testHtml | Out-File -FilePath $testHtmlPath -Encoding UTF8

Write-Host "WebDriverクラステストを開始します..." -ForegroundColor Green
...
try {
    ...
    $sourceCode = $driver.GetSourceCode()
    Write-Host "ソースコード長: $($sourceCode.Length) 文字" -ForegroundColor Green
    ...
    $driver.SetElementText($inputElement.nodeId, "テスト入力")
    ...
    $screenshotPath = Join-Path $PSScriptRoot "test_screenshot.png"
    $driver.GetScreenshot("fullPage", $screenshotPath)
    Write-Host "フルページスクリーンショット保存: $screenshotPath" -ForegroundColor Green
    
    # 要素スクリーンショット
    $elementScreenshotPath = Join-Path $PSScriptRoot "element_screenshot.png"
    ...
    $driver.SetCookie("test-cookie", "test-value")
    $cookieValue = $driver.GetCookie("test-cookie")
    Write-Host "クッキー値: $cookieValue" -ForegroundColor Green
    
    # ローカルストレージ操作テスト
    Write-Host "`n=== ローカルストレージ操作テスト ===" -ForegroundColor Cyan
    $driver.SetLocalStorage("test-key", "test-value")
    $storageValue = $driver.GetLocalStorage("test-key")
