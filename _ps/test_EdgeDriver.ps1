# EdgeDriverクラステストファイル
# EdgeDriverクラスの各メソッドの機能をテストするためのスクリプト
...
<html>
<head>
    <title>EdgeDriver Test Page</title>
    <style>
        .test-class { color: green; }
        .hidden { display: none; }
        .visible { display: block; }
    </style>
</head>
<body>
    <h1 id="main-title">EdgeDriver Test Page</h1>
    <div class="test-class">This is a test div for Edge</div>
    <input type="text" id="test-input" name="test-input" value="initial value">
    <button id="test-button" onclick="alert('Edge button clicked!')">Click Me</button>
...
# テスト用HTMLファイルを保存
$testHtmlPath = Join-Path $PSScriptRoot "test_edge_page.html"
$testHtml | Out-File -FilePath $testHtmlPath -Encoding UTF8

Write-Host "EdgeDriverクラステストを開始します..." -ForegroundColor Green

try {
    # EdgeDriverインスタンスを作成
    Write-Host "EdgeDriverを初期化中..." -ForegroundColor Yellow
    ...
    # EdgeDriver固有の機能テスト
    Write-Host "`n=== EdgeDriver固有機能テスト ===" -ForegroundColor Cyan
    
    # Edge実行ファイルパス取得テスト
    Write-Host "Edge実行ファイルパス: $($driver.browser_exe_path)" -ForegroundColor Green
    
    # ユーザーデータディレクトリ取得テスト
    Write-Host "ユーザーデータディレクトリ: $($driver.browser_user_data_dir)" -ForegroundColor Green
    
    # Edge初期化状態確認
    Write-Host "Edge初期化状態: $($driver.is_edge_initialized)" -ForegroundColor Green
    ...
    $driver.SetElementText($inputElement.nodeId, "Edgeテスト入力")
    ...
    $screenshotPath = Join-Path $PSScriptRoot "edge_test_screenshot.png"
    $driver.GetScreenshot("fullPage", $screenshotPath)
    Write-Host "フルページスクリーンショット保存: $screenshotPath" -ForegroundColor Green
    
    # 要素スクリーンショット
    $elementScreenshotPath = Join-Path $PSScriptRoot "edge_element_screenshot.png"
    ...
    $driver.SetCookie("edge-test-cookie", "edge-test-value")
    $cookieValue = $driver.GetCookie("edge-test-cookie")
    Write-Host "クッキー値: $cookieValue" -ForegroundColor Green
    
    # ローカルストレージ操作テスト
    Write-Host "`n=== ローカルストレージ操作テスト ===" -ForegroundColor Cyan
    $driver.SetLocalStorage("edge-test-key", "edge-test-value")
    $storageValue = $driver.GetLocalStorage("edge-test-key")
