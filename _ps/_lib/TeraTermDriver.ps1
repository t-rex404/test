...
class TeraTermDriver
{
    ...
    # ログファイルパス
    static [string]$NormalLogFile = ".\TeraTermDriver_$($env:USERNAME)_Normal.log"
    ...
    [void] LogInfo([string]$message)
    {
        if ($global:Common)
        {
            try
            {
                $global:Common.WriteLog($message, "INFO", "TeraTermDriver")
            }
            catch
            {
        ...
        else
        {
            ...
            $this.AppendTextNoBom(([TeraTermDriver]::NormalLogFile), $line + [Environment]::NewLine)
        }
    ...
    [void] LogError([string]$errorCode, [string]$message)
    {
        ...
        {
            $line = "[$(Get-Date -Format 'yyyy/MM/dd HH:mm:ss')] $message"
            $this.AppendTextNoBom(([TeraTermDriver]::ErrorLogFile), $line + [Environment]::NewLine)
        }
