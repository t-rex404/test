class ChromeDriver : WebDriver
{
    ...
    {
        # ブラウザの実行ファイルのパスを取得
        $browser_exe_reg_key   = 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe\'
        $browser_user_data_dir = 'C:\temp\UserDataDirectoryForChrome\'
        try
        {
            # ブラウザの実行ファイルのパスを取得
            ...
            {
                throw 'ブラウザ実行ファイルが見つかりませんでした。使用するブラウザのインストール状況を確認してください。'
            }
        }
        catch  
        {
