...
class WordDriver
{
    ...
    [void] LogInfo([string]$message)
    {
        if ($global:Common)
        {
            try
            {
                $global:Common.WriteLog($message, "INFO", "WordDriver")
            }
            catch
            {
        ...
        else
        {
            ...
            $this.AppendTextNoBom(([WordDriver]::NormalLogFile), $line + [Environment]::NewLine)
        }
    ...
    [void] LogError([string]$errorCode, [string]$message)
    {
        ...
        {
            $line = "[$(Get-Date -Format 'yyyy/MM/dd HH:mm:ss')] $message"
            $this.AppendTextNoBom(([WordDriver]::ErrorLogFile), $line + [Environment]::NewLine)
        }
