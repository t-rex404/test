...
class EdgeDriver
{
    ...
    EdgeDriver()
    {
        write-host 'EdgeDriver.psm1:'$PSScriptRoot
        # Edgeの実行ファイルのパスを取得
        ...
        $url = 'about:blank'
        $this.edge_user_data_dir = 'C:\temp\UserDataDirectoryForEdge\'
        #$argument_list = '--remote-debugging-port=9222 --disable-popup-blocking --no-first-run --disable-fre --user-data-dir=' + $this.edge_user_data_dir
        $argument_list = '--new-window ' + $url + ' --remote-debugging-port=9222 --disable-popup-blocking --no-first-run --disable-fre --user-data-dir=' + $this.edge_user_data_dir
        # Edgeをデバッグモードで開く
        ...
        $edge_exe_process = Start-Process -FilePath $edge_exe_path -ArgumentList $argument_list -PassThru
            # 引数の意味
        ...
        foreach ($tab in $tabs)
        {
            ...
            if ($tab.title -like $url -and $tab.type -like 'page')
            {
...
#Export-ModuleMember -Function * -Alias * -Variable * -Cmdlet * -Class 'EdgeDriver'
